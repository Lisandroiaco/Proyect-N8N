...
async function sendTransactionalMail(email, subject, html) {
...
function attachActivity(db, entry) {
    db.activityLogs.unshift(entry);
    db.activityLogs = db.activityLogs.slice(0, 1000);
}
async function resolveAuth(request, response, next) {
...
export function requireAuth(request, response, next) {
    void resolveAuth(request, response, next);
}
...
async function issueSession(response, db, user, request, rememberMe) {
    const session = {
        id: nanoid(),
        userId: user.id,
        refreshTokenHash: '',
...
async function getCurrentUserAndSession(request) {
...
router.post('/auth/register', authLimiter, requireCsrf, async (request, response) => {
    ...
    await writePlatformDatabase(db);
    const mailSent = await sendTransactionalMail(user.email, 'Verifica tu cuenta', `<p>Usa este token para verificar tu cuenta:</p><pre>${verificationToken}</pre>`);
...
router.post('/auth/login', authLimiter, requireCsrf, async (request, response) => {
    ...
    const payload = schema.parse(request.body);
    const db = await readPlatformDatabase();
    const user = db.users.find((item) => item.email.toLowerCase() === payload.identifier.toLowerCase() || item.username.toLowerCase() === payload.identifier.toLowerCase());
...
router.post('/auth/verify-email', requireCsrf, async (request, response) => {
    ...
    const payload = schema.parse(request.body);
    const db = await readPlatformDatabase();
    const token = db.authTokens.find((item) => item.type === 'email-verification' && item.tokenHash === hashOpaqueToken(payload.token) && !item.consumedAt);
...
router.post('/auth/reset-password', requireCsrf, async (request, response) => {
    ...
    const payload = schema.parse(request.body);
    const db = await readPlatformDatabase();
    const resetToken = db.authTokens.find((item) => item.type === 'password-reset' && item.tokenHash === hashOpaqueToken(payload.token) && !item.consumedAt);
...
router.post('/auth/magic-link/login', requireCsrf, async (request, response) => {
    ...
    const payload = schema.parse(request.body);
    const db = await readPlatformDatabase();
    const magicToken = db.authTokens.find((item) => item.type === 'magic-link' && item.tokenHash === hashOpaqueToken(payload.token) && !item.consumedAt);
...
router.get('/profile/ranking', async (_request, response) => {
    ...
    const ranking = db.users
        ...
        username: user.username,
        name: user.name,
        score: db.followers.filter((item) => item.followingUserId === user.id).length * 3 +
