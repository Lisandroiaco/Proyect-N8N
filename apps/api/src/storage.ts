...
function createStarterWorkflow(): WorkflowDefinition {
...
function normalizeWorkflows(workflows: WorkflowDefinition[]) {
    ...
    nodes: workflow.nodes.map((node) => ({
      ...
      data: {
        ...
        config: Object.fromEntries(
          Object.entries(node.data.config).filter(([key]) => !getRuntimeOnlyKeys(node.type).includes(key))
        )
      }
    }))
  }));
...
export async function readWorkflows(): Promise<WorkflowDefinition[]> {
  await ensureStorage();
  const content = await readFile(workflowsFile, 'utf8');
  return normalizeWorkflows(JSON.parse(content) as WorkflowDefinition[]);
}

export async function writeWorkflows(workflows: WorkflowDefinition[]) {
  await ensureStorage();
