...
function sanitizeRecipient(entry: ChatHistoryEntry) {
...
function sanitizeChatHistory(entries: ChatHistoryEntry[]) {
...
export async function readChatHistory(): Promise<ChatHistoryEntry[]> {
  await ensureChatStorage();
  const content = await readFile(chatHistoryFile, 'utf8');
  const parsed = JSON.parse(content) as ChatHistoryEntry[];
...
export async function appendChatHistory(entries: ChatHistoryEntry[]) {
  if (entries.length === 0) {
    return;
  }
